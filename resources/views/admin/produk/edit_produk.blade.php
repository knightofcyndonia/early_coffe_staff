...
<div class="container-fluid">   
  ...
  <div class="card card-default">
    <!-- /.card-header -->
    <form method="POST" action="{{url ('admin/edit_produk')}}">
    @csrf
    <input type="hidden" name="txtprodukid" id="txtprodukid" value="{{request()->id}}">
    <div class="card-body">
      <div class="row">
        <div class="col-md-12">
          <div class="form-group">
            <label>Nama</label>
                <input class="form-control" type="text" placeholder="Nama" id="txtNama" name="txtNama" value="{{$produkData['nama']}}">
          </div>
          <!-- /.form-group -->
        </div>
        <!-- /.col -->
        <div class="col-md-12">
          <div class="form-group">
            <label>Jurusan</label>
            <select class="form-control select2bs4" style="width: 100%;" id="ddlJurusan" name="ddlJurusan" value="{{$produkData['id_jurusan']}}" onchange="fnJurusanOnChange(this.value)">
              @foreach($listjurusan as $jurusan)
                <option value="{{$jurusan['id']}}" {{$jurusan['id'] == $produkData['id_jurusan'] ? 'SELECTED' : ''}}>{{$jurusan['jurusan']}}</option>
              @endforeach
            </select>
          </div>
          <!-- /.form-group -->
        ...
        <div class="col-md-12">
          ...
          <label>Jenis Jas</label>
            <select class="form-control select2bs4" style="width: 100%;" id="ddlJenis" name="ddlJenis">
              <option value="LAB" {{'LAB' == $produkData['jenis_jas'] ? 'SELECTED' : ''}}>Lab</option>
              <option value="ALMAMATER" {{'ALMAMATER' == $produkData['jenis_jas'] ? 'SELECTED' : ''}}>ALMAMATER</option>
            </select>
          </div>

          <div class="form-group">
            <label for="">Deskripsi</label>
            <textarea name="txtdeskripsi" id="txtdeskripsi" cols="30" rows="10" class="form-control" maxlength="500">{{$produkData['deskripsi']}}</textarea>
          </div>
          ...
          <table class="table table table-bordered " id="tbl_size_and_stock">
            ...
            <tbody>
            @foreach($listStok as $key=>$stok)
            <tr class="trproduk">
                <td style="width:2%!important" class="tdno">{{ ++$key}}</td>
                <td style="width:25%!important" class="tdsize">
                <input type="text" id="txtsize_{{$key}}" name="txtsize[]" class="txtsize form-control" maxlength="5" value="{{$stok['size']}}"/>
                </td>
                <td style="width:25%!important" class="tdstok">
                <input type="text" id="txtstok_{{$key}}" name="txtstok[]" class="txtstok form-control" maxlength="11" onkeypress="validate(event)"  value="{{$stok['stok']}}"\>
                </td>
                <td style="width:25%!important" class="tddetail">
                <input type="text" id="txtdetail_{{$key}}" name="txtdetail[]" class="txtdetail form-control" maxlength="100" value="{{$stok['detail']}}">
                </td>
                <td class="tdaksi" style="width:15%!important">
                    <button type="button" id="btn_delete_{{$key}}" class="btn btn-danger btn_delete btn-block" onclick="fnDeleteRow(this, 'trproduk')"><i class="fas fa-trash"></i> Hapus </button>
                </td>
            </tr>
            @endforeach              
            </tbody>
          ...
          <div class="row">
              <div class="col-md-2">
              <button type="button" onclick="fnAddUpload()" class="form-control btn btn-small btn-primary" style=" margin-top : 20px; margin-bottom:10px">Upload Gambar  <i class="fa fa-plus-circle "></i></button>
              </div>
          </div>
          <table class="table table-bordered" id="tbl_image">
            ...
            <tr class="trimage">
                <td class="tdNoGambar"style="width:2%!important">{{ ++$key}}</td>
                <td width="14%">
                    <input hidden id="gambar_{{$key}}" type="file" accept="image/*" class="input-gambar" onchange="convert('gambar_{{$key}}', 'base64_gambar_{{$key}}', 'output_gambar_{{$key}}')">
                    <input type="hidden" id="base64_gambar_{{$key}}" name="gambar[]" class="base64_gambar" value="{{$image['image']}}">
                </td>
                <td class="tdgambar"width="11%">
                  <img id="output_gambar_{{$key}}" class="p-3 output_gambar" style="max-width: 200px" src="{{image_url(). $image['image']}}"/>
                </td>
                <td style="width:5%!important">
                <button type="button" id="btn_delete_image_{{$key}}" class="btn btn-danger btn_delete_image btn-block" onclick="fnDeleteRow(this, 'trimage')"><i class="fas fa-trash"></i> Hapus </button>
                </td>
            </tr>
            @endforeach              
            </tbody>
            ...
            <tfoot hidden>
              <tr>
                <td style="width:2%!important">No</td>
                <td style="width:25%!important" colspan="2">Gambar</td>
                <td style="width:5%!important">
                  Aksi  
                </td>
              </tr>
...
<script>
  $(function () {
    ...
    // fnDataTable();
    var param_jurusan = {!! json_encode($produkData['id_prodi']) !!};
    fnJurusanOnChange($("#ddlJurusan").val());
    $("#nav-item-produk").addClass("active");
    $("#ddlProdi").val(param_jurusan);
  });
  ...
  function fnValidation()
  {
    ...
    var listinputgambar = $(".trimage:not('.trdeleted') .base64_gambar");
  ...
  {
    $($this).closest('tr').addClass("trdeleted");
    ...
    {
      $($this).closest('tr').children('.tdNoGambar').removeClass("tdNoGambar");
      $($this).closest('tr').hide();
    }

    var delete_gambar = $($($this).closest('tr')).find(".base64_gambar");
    $(delete_gambar).attr("name", "listDeletedImage[]");
  ...
  function fnAddUpload($this)
  {
    ...
    $("#tbl_image tbody").append('\
      <tr class="trimage">\
          ...
          <td >\
              ...
              <input type="hidden" id="base64_gambar_'+no+'" name="listInsertedImage[]" class="base64_gambar" value="">\
          </td>\
          <td  class="tdgambar">\
    ...
    reorder_number('trimage');
  }

  function reorder_number(className)
  {
    var length = $("."+className+ ":not(.trdeleted)").length;
    var count = 1;
